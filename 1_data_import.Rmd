...
to_table1 <-
  df_hsi %>% 
    ...
    dplyr::select(source, age, sex, race_ethnic, service, grade, hsi, bmi, rhabdo, casedef) %>% 
...
table1::table1(~ hsi + sex + age + sex + grade + race_ethnic + rhabdo + bmi + casedef | source, data = to_table1)
...
to_table1_casedef <-
  df_hsi %>% 
    filter(casedef == 1) %>% 
    dplyr::select(source, age, sex, race_ethnic, service, grade, hsi, bmi, rhabdo) %>% 
       mutate(service = recode(service,
            "A" = "Army",
            "C" = "Coast Guard",
            "F" = "Air Force",
...
table1::table1(~ hsi + sex + age + sex + grade + race_ethnic + rhabdo + bmi | source, data = to_table1_casedef)
