...
to_table1 <-
  df_hsi %>% 
    ...
    dplyr::select(source, age, sex, race_ethnic, service, grade, hsi, bmi, rhabdo, casedef, bmi_cat, decade) %>% 
...
to_table1_casedef <-
  df_hsi %>% 
    filter(casedef == 1) %>% 
    dplyr::select(source, age, sex, race_ethnic, service, grade, hsi, bmi, rhabdo, bmi_cat, decade) %>% 
       mutate(service = recode(service,
            "A" = "Army",
            "C" = "Coast Guard",
            "F" = "Air Force",
